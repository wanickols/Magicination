...
namespace MGCNTN
{
    ...
    public static class Extensions
    {
        ...
        public static void applyStatusEffect(this List<Status> statuses)
        {

        }

        ///AugmentStats
        public static void tickDuration(this List<AugmentStats> augmentStats)
        {
            for (int i = augmentStats.Count - 1; i >= 0; i--)
                if (--augmentStats[i].duration <= 0)
                {
                    //Tell Origin Object removing
                    augmentStats.RemoveAt(i);
                }
