...
public class CutsceneEditor : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        VisualTreeAsset original = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Assets/Editor/EditMode/Windows/CutsceneEditor.uxml");
        TemplateContainer treeAsset = original.CloneTree();
        rootVisualElement.Add(treeAsset);

        // Find the .uss stylesheet and set this window to use it
        StyleSheet styleSheet = AssetDatabase.LoadAssetAtPath<StyleSheet>("Assets/Editor/EditMode/Windows/CutsceneEditor.uss");
        ...
        // Find all classes we have that implement ICutCommand and store them in a list
    ...
    private List<ICutCommand> commands;
    ...
    private void InitializeWindow()
    {
        ...
        addButton.clicked += () =>
        {
            ...
            if (commandList.selectedItem != null)
                cutscene.InsertCommand(commandPosition.value, CreateCommand((ICutCommand)commandList.selectedItem));
    ...
    // Finds all classes that implement ICutCommand
    private List<ICutCommand> FindAllSceneCommands()
    {
        var type = typeof(ICutCommand);

        return AppDomain.CurrentDomain.GetAssemblies()
            .SelectMany(assembly => assembly.GetTypes())
            .Where(assembly => type.IsAssignableFrom(assembly) && !assembly.IsInterface)
            .Select(type => Activator.CreateInstance(type) as ICutCommand)
            .ToList();
    }

    // Creates a new instance of the passed in command
    private ICutCommand CreateCommand(ICutCommand command)
    {
